...
const formatServerResponse = (serverResponse: ReturnType<ServerResponse[]>): ReturnType<DisciplineType[]> => {
    const formattedServerResponse: DisciplineType[] =
        serverResponse.data?.map(item => ({
            ...item,
            numberOfChildren: item.total_discipline_children > 0 ? item.total_discipline_children :
                item.total_training_sets > 0 ? item.total_training_sets : item.total_documents > 0 ? item.total_documents : 0,
