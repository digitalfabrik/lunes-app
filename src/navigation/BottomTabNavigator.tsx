...
const BottomTabNavigator = (): JSX.Element | null => {
    ...
    const {data: repetitionBadgeNumber } = useLoadAsync(getNumberOfWordNodeCardsNeedingRepetitionWithUpperBound, undefined)

    const renderHomeTabIcon = ({focused}: { focused: boolean }) =>
        focused ? <HomeIconWhite width={hp('5%')} height={hp('5%')}/> :
            <HomeIconGrey width={hp('5%')} height={hp('5%')}/>

    const renderFavoritesTabIcon = ({focused}: { focused: boolean }) =>
        focused ? <StarIconWhite width={iconSize} height={iconSize}/> :
            <StarIconGrey width={iconSize} height={iconSize}/>
